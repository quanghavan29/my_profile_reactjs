                ...
                <Grid item xs={12} style={{ paddingTop: 40 }}>
                    <Grid container spacing={3}>
                        {resumeData.projects.map((project, index) => (
                            <>
                                {project.tag.includes(tabValue) || tabValue === 'All' ? (<Grid item xs={12} sm={6} md={3} lg={4}>
                                    <Grow in timeout={1000}>
                                        <Card className="custom_card" onClick={() => { setProjectDialog(project) }}>
                                            <CardActionArea>
                                                ...
                                                <CardContent>
                                                    <Typography className="custom_card_title">
                                                        {project.title}
                                                        {project.tag === "NodeJS and ReactJS" ? 
                                                        <StarsIcon style={{ marginLeft: 8, color: "#FFCA28" }} /> 
                                                        : null}
                                                    </Typography>
                                                    <Typography className="custom_card_caption" variant="body2">
                                                        {project.caption}
                                                    </Typography>
                                                </CardContent>
                                            </CardActionArea>
